...
def homebrew_rescale1(train_data, test_data):
    ...
    diag = 1 / np.diag(X_inv)
    recenter = np.exp(np.mean(np.log(diag)))
...
def homebrew_rescale2(train_data, test_data):
    ...
    print(print_coef)
    diagA = np.diag(np.array(model.normalized_cov_params))
    diagB = (diagA * (diagA < 10)) + (0.5 * (min(diagA)) * (diagA > 10))
    recenter = np.exp(np.mean(np.log(diagB[1:])))
...
print(f1_score(y_test, logpred1zE))
...
print(f1_score(y_test, logpred1aE))
...
print(f1_score(y_test, logpred1bE))
...
print(f1_score(y_test, logpred2zE_test))
...
print(f1_score(y_test, logpred2aE_test))
...
print(f1_score(y_test, logpred2bE_test))
